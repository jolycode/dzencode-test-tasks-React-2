...
import { X, Calendar, AlertCircle, Monitor, ChevronLeft, ChevronRight, Trash2, Plus, ArrowLeft } from 'lucide-react';
import { productsAPI, Product, CreateProductData } from '../services/api';

interface TypesGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  typeName: string;
...
type ModalMode = 'list' | 'add';

export const TypesGroupModal: React.FC<TypesGroupModalProps> = ({
  isOpen,
  onClose,
  typeName,
  onProductDeleted
  ...
  useEffect(() => {
    if (isOpen && typeName && modalMode === 'list') {
      fetchTypeProducts();
    }
  }, [isOpen, typeName, selectedSpecification, selectedStatus, currentPage, modalMode]);

  useEffect(() => {
    if (isOpen && modalMode === 'list') {
      setCurrentPage(1);
    }
  }, [selectedSpecification, selectedStatus]);
  ...
  const fetchFiltersForType = async () => {
    try {
      const response = await productsAPI.getProducts({ limit: 1000 });
      const typeProducts = response.data.filter(product => product.type === typeName);
      
      const specifications = [...new Set(typeProducts.map(p => p.specification).filter((spec): spec is string => typeof spec === 'string'))];
      const statuses = [...new Set(typeProducts.map(p => p.status).filter(Boolean))];
  ...
  const fetchTypeProducts = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch type products');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.usdPrice && !formData.uahPrice) {
        throw new Error('Укажите как минимум одну цену (USD или UAH)');
      }

      const prices = [];
      if (formData.usdPrice && Number(formData.usdPrice) > 0) {
        prices.push({
          value: Number(formData.usdPrice),
          symbol: 'USD',
          isDefault: formData.defaultCurrency === 'USD'
        });
      }
      if (formData.uahPrice && Number(formData.uahPrice) > 0) {
        prices.push({
          value: Number(formData.uahPrice),
          symbol: 'UAH',
          isDefault: formData.defaultCurrency === 'UAH'
        });
      }
      ...
      const productData: CreateProductData = {
        ...
        incomingGroup: formData.incomingGroup,
    ...
    <div className="flex-1 overflow-y-auto p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Группа прихода <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              name="incomingGroup"
        ...
        <div className="space-y-4">
          <h4 className="text-md font-medium text-gray-900">Цены</h4>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-3 md:space-y-0">
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="md:hidden bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center space-x-1 transition-colors"
              >
      ...
      <div className="h-full md:h-auto md:max-h-[90vh] md:mt-8 md:mb-8 md:mx-4 md:flex md:items-center md:justify-center">
        <div className="bg-white h-full md:h-auto md:rounded-lg md:max-w-7xl w-full md:max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 bg-gray-50 flex-shrink-0">
              ...
              <div className="flex items-center space-x-2">
                {modalMode === 'add' && (
                  <button
                    ...
                    title="Назад к списку продуктов"
                  >
                ...
                <div>
                  <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-1">
                    {modalMode === 'add' ? `Добавить продукт типа "${typeName}"` : 'Продукты типа'}
                  </h2>
                  {modalMode === 'list' && (
                    <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-1 md:space-y-0">
                      <div className="text-sm text-gray-500">
                        Найдено: {pagination.totalItems} продуктов типа '{typeName}'
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="flex items-center space-x-2 ml-2">
              {modalMode === 'list' && (
                <button
                  onClick={switchToAddMode}
                  className="w-10 h-10 bg-green-600 hover:bg-green-700 text-white rounded-full flex items-center justify-center transition-colors shadow-lg hover:shadow-xl transform hover:scale-105"
              ...
              <button
                onClick={onClose}
                className="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded-full hover:bg-gray-200 flex-shrink-0"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {modalMode === 'add' ? renderAddProductForm() : renderProductsList()}

            <div className="border-t border-gray-200 px-4 md:px-6 py-4 bg-gray-50 flex justify-end space-x-3 flex-shrink-0">
              {modalMode === 'add' ? (
                <>
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    disabled={addProductLoading}
                    className="px-6 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 flex items-center"
                  >
                    {addProductLoading ? (
                      <>
